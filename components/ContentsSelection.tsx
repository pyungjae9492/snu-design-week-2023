...
export const ContentsSelection = ({ headingList, contentsByHeading }) => {
    ...
    <>  
        ...
        <div className={"overflow-hidden relative flex justify-center items"} style={heightByElementSize}>
            {contentsByHeading.map((content:()=>JSX.Element, index:number) => (
                <div key={index} className={`${selectedIndex === index ? "opacity-100" : "opacity-0"} absolute transition-opacity duration-500 ease-in-out w-full max-w-[900px] sm:max-w-[345px] sm:justify-center sm:mt-[150px]`} ref={(ref) => {wholeContentsRef.current[index] = ref}}>
                    {content()}
                </div>
            ))}
        </div>
