...
export const ContentsSelection = ({ headingList, contentsByHeading }) => {
    ...
    <>  
        ...
        <div className={"overflow-hidden relative flex flex-col justify-center items-center"} style={heightByElementSize}>
            {contentsByHeading.map((content:()=>JSX.Element, index:number) => (
                <div key={index} className={`${selectedIndex === index ? "opacity-100" : "opacity-0"} absolute transition-opacity duration-500 ease-in-out w-full max-w-[900px] sm:w-[90%] sm:justify-center sm:ml-[5%] sm:mt-[150px]`} ref={(ref) => {wholeContentsRef.current[index] = ref}}>
                    {content()}
                </div>
            ))}
        </div>
